...
Client.prototype.downloadFile = function (identifier) {
    var self = this;
    if (identifier && identifier.download_url){
        return download(identifier)
    } else {
        return self.getFileInformation(identifier,{},{transform:false}).then(download)
    }
    function download(meta_data){
        var deferred = q.defer();
        request.get({
                headers: {
...
Client.prototype.searchFiles = function (filters,options) {
    ...
    if (filters.mimetype){
        query.push("mimeType='"+filters.mimetype+"'");
    } else {
        query.push("mimeType!='application/vnd.google-apps.folder'");
    }
    if (filters.parent_folder){
        query.push("'"+filters.parent_folder+"' in parents");
    }
    // Collect the parameters that will be passed to list()
    var params = {};
    params.max_results = options.max_results || 100;
    if (query.length){
    ...
    if (options.fields){
        // TODO: anti-generalize the options.fields array
        // before passing it to Google Drive
        params.fields = "items("+options.fields.join()+")";
